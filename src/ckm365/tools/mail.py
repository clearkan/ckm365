...
def _write_target(dest_path: str, dir_name: str | None) -> Path:
...
def _write_atomic(dest: Path, produce) -> int:
...
def download_attachment(ctx: Ctx, message_id: str, dest_path: str, *,
    ...
    found = _select_attachment(_attachments(g, mb, message_id),
                               attachment_id, name)
    dest = _write_target(dest_path, found.name)
    written = _write_atomic(dest, lambda part: g.download(_message_path(
        mb, message_id,
...
def export_message(ctx: Ctx, message_id: str, dest_path: str, *,
    ...
    if suffix not in _EXPORT_TEXT | _EXPORT_RAW:
        raise ValueError(
            ...
            ".eml for raw MIME)")
    g, mb = ctx.target(account, mailbox)
    dest = _write_target(dest_path, None)
    count: int | None = None
    if suffix in _EXPORT_RAW:
        written = _write_atomic(
            dest, lambda part: g.download(_message_path(mb, message_id,
                                                        "/$value"), part))
    else:
        message = Message.from_graph(
            g.get(_message_path(mb, message_id),
                  params={"$select": Message.SELECT}, headers=_prefer("text")))
        items = _attachments(g, mb, message_id) if message.has_attachments else []
        record = _record(message, items, mb, __version__).encode("utf-8")
        written = _write_atomic(dest, lambda part: part.write_bytes(record))
        count = len(items)
